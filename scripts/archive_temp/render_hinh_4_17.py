...
def render_hinh_4_17_admin_catalogue_seq(draw, style="infographic"):
    W, H = 2000, 1300
    draw_header(draw, "SƠ ĐỒ TUẦN TỰ QUẢN TRỊ DANH MỤC & KHO SÁCH (ADMIN)", "Quy trình kiểm soát phân quyền RBAC, thêm mới/cập nhật sách và đồng bộ dữ liệu", W)

    # Lifelines: Admin | Admin Portal UI | JwtAuthFilter (RBAC) | Book/Category Controller | Book/Category Service | PostgreSQL DB
    actors = [
        (160, "Administrator"),
        (450, "Admin UI (React)"),
        (780, "JwtAuthFilter"),
        (1130, "Book/Cat Controller"),
        (1480, "Book/Cat Service"),
        (1820, "PostgreSQL DB")
    ]

    for x, name in actors:
        draw_card(draw, [x - 120, 170, x + 120, 240], name, "", style, "purple" if "Admin" in name else "blue")
        draw.line([(x, 240), (x, 1140)], fill=(148, 163, 184), width=2)

    # Messages
    msgs = [
        (160, 450, "1. Nhập thông tin sách / Danh mục mới", 300),
        (450, 780, "2. POST/PUT /api/books (Bearer JWT)", 390),
        (780, 780, "3. Xác thực Token & Kiểm tra Role = ADMIN", 480),
        (780, 1130, "4. Chuyển tiếp Request đã xác thực", 570),
        (1130, 1480, "5. createBook(book) / updateBook()", 660),
        (1480, 1820, "6. bookRepository.save(book)", 760),
        (1820, 1480, "7. INSERT/UPDATE thành công", 860, True),
        (1480, 1480, "8. Xóa Cache (@CacheEvict)", 930),
        (1480, 1130, "9. Trả về Entity Book/Category", 1000, True),
        (1130, 450, "10. HTTP 200 OK / 201 Created", 1060, True),
        (450, 160, "11. Cập nhật bảng Kho sách & Báo thành công", 1120, True)
    ]
...
def generate_hinh_4_17():
    # 1. Render Infographic Revised
    img_info = Image.new("RGB", (2000, 1300), (255, 255, 255))
    draw_info = ImageDraw.Draw(img_info)
    render_hinh_4_17_admin_catalogue_seq(draw_info, style="infographic")
    p_info = os.path.join(INFO_REV_DIR, "Hinh_4.17_Sequence_Diagram_Admin_Catalogue.png")
    ...
    render_hinh_4_17_admin_catalogue_seq(draw_std, style="standard")
    p_std = os.path.join(STD_REV_DIR, "Hinh_4.17_Sequence_Diagram_Admin_Catalogue.png")
    img_std.save(p_std, dpi=(300, 300))

    # SVG stubs
    svg_content = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 1300" width="2000" height="1300">\n  <rect width="100%" height="100%" fill="#ffffff"/>\n  <!-- Vector source for Hinh_4.17_Sequence_Diagram_Admin_Catalogue -->\n</svg>'
    with open(os.path.join(INFO_REV_DIR, "Hinh_4.17_Sequence_Diagram_Admin_Catalogue.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)
    with open(os.path.join(STD_REV_DIR, "Hinh_4.17_Sequence_Diagram_Admin_Catalogue.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)

    print("Generated Hinh 4.17 in both revised directories successfully!")
