...
def render_hinh_4_9_search_filter_seq(draw, style="infographic"):
    W, H = 2000, 1300
    draw_header(draw, "SƠ ĐỒ TUẦN TỰ TÌM KIẾM VÀ LỌC SÁCH NÂNG CAO", "Quy trình tìm kiếm theo từ khóa đa trường và lọc kết hợp thể loại, giá", W)

    # Lifelines: Customer | Search & Filter UI | BookController | BookService | BookRepository | PostgreSQL DB
    actors = [
        (180, "Customer"),
        (480, "Search UI (React)"),
        (820, "BookController"),
    ...
    for x, name in actors:
        draw_card(draw, [x - 130, 170, x + 130, 240], name, "", style, "purple" if "Customer" in name or "React" in name else "blue")
        draw.line([(x, 240), (x, 1140)], fill=(148, 163, 184), width=2)

    # Messages
    msgs = [
        (180, 480, "1. Nhập từ khóa / Chọn bộ lọc (thể loại, giá)", 300),
        (480, 820, "2. GET /api/books/search?keyword={kw}", 400),
        (820, 1160, "3. searchBooks(keyword)", 500),
        (1160, 1160, "4. Chuẩn hóa & loại bỏ stop-words", 590),
        (1160, 1500, "5. searchBooksByKeyword(cleanKeyword)", 680),
        (1500, 1820, "6. SELECT WHERE title/author/cat LIKE %kw%", 780),
        (1820, 1500, "7. Trả về tập kết quả bản ghi sách", 880, True),
        (1500, 1160, "8. List<Book> Entities", 960, True),
        (1160, 820, "9. Lọc theo khoảng giá & sắp xếp", 1030, True),
        (820, 480, "10. HTTP 200 OK (Danh sách sách khớp)", 1090, True),
        (480, 180, "11. Render lưới kết quả tìm kiếm & Phân trang", 1140, True)
    ]
...
def generate_hinh_4_9():
    # 1. Render Infographic Revised
    img_info = Image.new("RGB", (2000, 1300), (255, 255, 255))
    draw_info = ImageDraw.Draw(img_info)
    render_hinh_4_9_search_filter_seq(draw_info, style="infographic")
    p_info = os.path.join(INFO_REV_DIR, "Hinh_4.9_Sequence_Diagram_Search_Filter.png")
    ...
    render_hinh_4_9_search_filter_seq(draw_std, style="standard")
    p_std = os.path.join(STD_REV_DIR, "Hinh_4.9_Sequence_Diagram_Search_Filter.png")
    img_std.save(p_std, dpi=(300, 300))

    # SVG stubs
    svg_content = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 1300" width="2000" height="1300">\n  <rect width="100%" height="100%" fill="#ffffff"/>\n  <!-- Vector source for Hinh_4.9_Sequence_Diagram_Search_Filter -->\n</svg>'
    with open(os.path.join(INFO_REV_DIR, "Hinh_4.9_Sequence_Diagram_Search_Filter.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)
    with open(os.path.join(STD_REV_DIR, "Hinh_4.9_Sequence_Diagram_Search_Filter.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)

    print("Generated Hinh 4.9 in both revised directories successfully!")
