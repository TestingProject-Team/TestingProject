...
def render_hinh_4_11_cart_seq(draw, style="infographic"):
    W, H = 2000, 1300
    draw_header(draw, "SƠ ĐỒ TUẦN TỰ THAO TÁC GIỎ HÀNG (CART MANAGEMENT)", "Quy trình thêm sách, cập nhật số lượng, kiểm tra tồn kho và đồng bộ giỏ hàng", W)

    # Lifelines: Customer | Cart UI | CartController | CartService | Cart/Book Repository | PostgreSQL DB
    actors = [
        (180, "Customer"),
        (480, "Cart UI (React)"),
        (820, "CartController"),
    ...
    for x, name in actors:
        draw_card(draw, [x - 130, 170, x + 130, 240], name, "", style, "blue" if "Cart" in name or "React" in name else "teal")
        draw.line([(x, 240), (x, 1140)], fill=(148, 163, 184), width=2)

    # Messages
    msgs = [
        (180, 480, "1. Bấm 'Thêm vào giỏ hàng' / Cập nhật số lượng", 300),
        (480, 820, "2. POST /api/cart {bookId, quantity}", 400),
        (820, 1160, "3. addToCart(username, CartRequest)", 500),
        (1160, 1500, "4. findByUserId() & bookRepo.findById()", 600),
        (1500, 1820, "5. SELECT * FROM carts, books WHERE id = ?", 700),
        (1820, 1500, "6. Trả về thực thể Cart & Book", 800, True),
        (1500, 1160, "7. Kiểm tra tồn kho & Thêm CartItem", 890, True),
        (1160, 1500, "8. cartRepository.save(cart)", 970),
        (1500, 1820, "9. UPDATE/INSERT cart_items INTO DB", 1030),
        (1160, 820, "10. Trả về Cart Entity mới nhất", 1080, True),
        (820, 480, "11. HTTP 200 OK (Cart DTO)", 1115, True),
        (480, 180, "12. Cập nhật Badge số lượng & Tổng tiền giỏ hàng", 1145, True)
    ]
...
def generate_hinh_4_11():
    # 1. Render Infographic Revised
    img_info = Image.new("RGB", (2000, 1300), (255, 255, 255))
    draw_info = ImageDraw.Draw(img_info)
    render_hinh_4_11_cart_seq(draw_info, style="infographic")
    p_info = os.path.join(INFO_REV_DIR, "Hinh_4.11_Sequence_Diagram_Cart_Management.png")
    ...
    render_hinh_4_11_cart_seq(draw_std, style="standard")
    p_std = os.path.join(STD_REV_DIR, "Hinh_4.11_Sequence_Diagram_Cart_Management.png")
    img_std.save(p_std, dpi=(300, 300))

    # SVG stubs
    svg_content = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 1300" width="2000" height="1300">\n  <rect width="100%" height="100%" fill="#ffffff"/>\n  <!-- Vector source for Hinh_4.11_Sequence_Diagram_Cart_Management -->\n</svg>'
    with open(os.path.join(INFO_REV_DIR, "Hinh_4.11_Sequence_Diagram_Cart_Management.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)
    with open(os.path.join(STD_REV_DIR, "Hinh_4.11_Sequence_Diagram_Cart_Management.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)

    print("Generated Hinh 4.11 in both revised directories successfully!")
