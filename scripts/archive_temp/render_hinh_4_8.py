...
def render_hinh_4_8_product_list_seq(draw, style="infographic"):
    W, H = 2000, 1300
    draw_header(draw, "SƠ ĐỒ TUẦN TỰ XEM DANH MỤC VÀ DANH SÁCH SÁCH", "Quy trình tải cây danh mục thể loại và truy vấn danh sách sách theo điều kiện", W)

    # Lifelines: Customer | Storefront UI | BookController | BookService | BookRepository | PostgreSQL
    actors = [
        (180, "Customer"),
        (480, "Storefront UI (React)"),
        (820, "BookController"),
        (1160, "BookService"),
        (1500, "BookRepository"),
        (1820, "PostgreSQL DB")
    ...
    for x, name in actors:
        draw_card(draw, [x - 130, 170, x + 130, 240], name, "", style, "teal" if "Customer" in name or "React" in name else "blue")
        draw.line([(x, 240), (x, 1140)], fill=(148, 163, 184), width=2)

    # Messages
    msgs = [
        (180, 480, "1. Truy cập Trang chủ / Khám phá sách", 300),
        (480, 820, "2. GET /api/categories & GET /api/books", 400),
        (820, 1160, "3. getAllCategories() & getAllBooks()", 500),
        (1160, 1500, "4. findAll() / findByCategoryId()", 600),
        (1500, 1820, "5. SELECT * FROM categories, books", 700),
        (1820, 1500, "6. Trả về kết quả truy vấn", 800, True),
        (1500, 1160, "7. Danh sách Entity Category & Book", 890, True),
        (1160, 820, "8. Map sang List<BookDTO>", 970, True),
        (820, 480, "9. HTTP 200 OK (JSON Data)", 1040, True),
        (480, 180, "10. Render danh mục & Lưới sách nổi bật", 1110, True)
    ]
...
def generate_hinh_4_8():
    # 1. Render Infographic Revised
    img_info = Image.new("RGB", (2000, 1300), (255, 255, 255))
    draw_info = ImageDraw.Draw(img_info)
    render_hinh_4_8_product_list_seq(draw_info, style="infographic")
    p_info = os.path.join(INFO_REV_DIR, "Hinh_4.8_Sequence_Diagram_Product_List.png")
    ...
    render_hinh_4_8_product_list_seq(draw_std, style="standard")
    p_std = os.path.join(STD_REV_DIR, "Hinh_4.8_Sequence_Diagram_Product_List.png")
    img_std.save(p_std, dpi=(300, 300))

    # SVG stubs
    svg_content = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 2000 1300" width="2000" height="1300">\n  <rect width="100%" height="100%" fill="#ffffff"/>\n  <!-- Vector source for Hinh_4.8_Sequence_Diagram_Product_List -->\n</svg>'
    with open(os.path.join(INFO_REV_DIR, "Hinh_4.8_Sequence_Diagram_Product_List.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)
    with open(os.path.join(STD_REV_DIR, "Hinh_4.8_Sequence_Diagram_Product_List.svg"), "w", encoding="utf-8") as f:
        f.write(svg_content)

    print("Generated Hinh 4.8 in both revised directories successfully!")
